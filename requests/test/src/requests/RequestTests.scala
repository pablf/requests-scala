...
object RequestTests extends HttpbinTestSuite {
  ...
  val tests = Tests{
    ...
    test("params"){
      test("get"){
        // All in URL
        val res1 = requests.get(s"https://$localHttpbin/get?hello=world&foo=baz").text()
        assert(read(res1).obj("args") == Obj("foo" -> "baz", "hello" -> "world"))

        // All in params
        val res2 = requests.get(
          s"https://$localHttpbin/get",
        ...
        val res3 = requests.get(
          s"https://$localHttpbin/get?hello=world",
          params = Map("foo" -> "baz")
        ).text()
    ...
    test("cookies"){
      test("session"){
        ...
        val res1 = s.get(s"https://$localHttpbin/cookies").text().trim
        assert(read(res1) == Obj("cookies" -> Obj("hello" -> "world")))
        s.get(s"https://$localHttpbin/cookies/set?freeform=test")
        val res2 = s.get(s"https://$localHttpbin/cookies").text().trim
        assert(read(res2) == Obj("cookies" -> Obj("freeform" -> "test", "hello" -> "world")))
      }
      test("raw"){
        val res1 = requests.get(s"https://$localHttpbin/cookies").text().trim
        assert(read(res1) == Obj("cookies" -> Obj()))
        requests.get(s"https://$localHttpbin/cookies/set?freeform=test")
        val res2 = requests.get(s"https://$localHttpbin/cookies").text().trim
      ...
      test("space"){
        ...
        s.get(s"https://$localHttpbin/cookies/set?freeform=test+test")
        val res2 = s.get(s"https://$localHttpbin/cookies").text().trim
    ...
    test("redirects"){
      test("max"){
        val res1 = requests.get(s"https://$localHttpbin/absolute-redirect/4")
        assert(res1.statusCode == 200)
        val res2 = requests.get(s"https://$localHttpbin/absolute-redirect/5")
        assert(res2.statusCode == 200)
        val res3 = requests.get(s"https://$localHttpbin/absolute-redirect/6", check = false)
        assert(res3.statusCode == 302)
        val res4 = requests.get(s"https://$localHttpbin/absolute-redirect/6", maxRedirects = 10)
        assert(res4.statusCode == 200)
      }
      test("maxRelative"){
        val res1 = requests.get(s"https://$localHttpbin/relative-redirect/4")
        assert(res1.statusCode == 200)
        val res2 = requests.get(s"https://$localHttpbin/relative-redirect/5")
        assert(res2.statusCode == 200)
        val res3 = requests.get(s"https://$localHttpbin/relative-redirect/6", check = false)
        assert(res3.statusCode == 302)
        val res4 = requests.get(s"https://$localHttpbin/relative-redirect/6", maxRedirects = 10)
        assert(res4.statusCode == 200)
      }
    }

    test("test_reproduction"){
      requests.get(s"https://$localHttpbin/status/304").text()

    }
    test("streaming"){
      val res1 = requests.get(s"https://$localHttpbin/stream/5").text()
      assert(res1.linesIterator.length == 5)
      val res2 = requests.get(s"https://$localHttpbin/stream/52").text()
    ...
    test("timeouts"){
      test("read"){
        intercept[TimeoutException] {
          requests.get(s"https://$localHttpbin/delay/1", readTimeout = 10)
        }
        requests.get(s"https://$localHttpbin/delay/1", readTimeout = 5000)
        intercept[TimeoutException] {
          requests.get(s"https://$localHttpbin/delay/3", readTimeout = 2000)
        }
      }
      test("connect"){
        intercept[TimeoutException] {
    ...
    test("decompress"){
      val res1 = requests.get(s"https://$localHttpbin/gzip")
      assert(read(res1.text()).obj("headers").obj("Host").str == localHttpbin)

      val res2 = requests.get(s"https://$localHttpbin/deflate")
      assert(read(res2).obj("headers").obj("Host").str == localHttpbin)

      val res3 = requests.get(s"https://$localHttpbin/gzip", autoDecompress = false)
      assert(res3.bytes.length < res1.bytes.length)

      val res4 = requests.get(s"https://$localHttpbin/deflate", autoDecompress = false)
    ...
    test("headers"){
      test("default"){
      val res = requests.get(s"https://$localHttpbin/headers").text()
    ...
    test("duplicateHeaders"){
      val res = requests.get(s"https://$localHttpbin/get", headers = Seq("x-y" -> "a", "x-y" -> "b"))
